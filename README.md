...
-- Where you fix a lot of issues that be present in the raw data so that when you start creating visualizations and start using it in your products, 
-- You can be sure that the data is actually useful and there are not a lot of issues with it
-- First of all, you will need to create a database, then import a dataset, and then, go ahead to clean the data
-- There are various steps you should take in order to clean this data
-- Create New Database: Go up to the left-hand corner and click on 'create a new schema in the connected server'
-- A tab opens up. Here, you have to name the database. In this case, I named this database 'world_layoffs'
-- So, you can already tell that I am working with a layoffs dataset
-- Next, click 'Apply'. It takes you to another tab session. Click 'Apply' again. It takes you to another tab session. Click 'Finish' 
-- and that creates the world_layoffs database
...
-- Select the layoffs dataset or whichever project saved on your computer as the case may be, and then click 'Next'
-- Next thing is to select destination. You can see that the name I gave the database is there already.
-- You can decide to also check the box that says 'Drop table if exists' just in case. Then click 'Next'
-- Next is to configure import settings. This is where the data shows up and categorised with their respective data types
-- Now, MySQL automatically assigns the data type based off of the data in this columns. 
-- Now, there is one thing to notice here though. 
-- There is a date column in this dataset and if you look at the data type, 
-- you will notice it's classified as a text because of the format it is in, in the dataset
-- But, we're going to import this as the raw data, and not change anything in the import settings because we are assuming this is how it came with the data.
...
-- It has the different companies that did the layoffs, the location of where they are, what industries they are part of, how many in total were laid off, 
-- the percentage they laid off, the date, the stage which refers to the stage the company's in (whether it's a Series B, Unknown, Post-IPO, etc,.), 
-- the countries, and then the funds raised in millions
...
-- Now, in the real workplace, often times, you have processes that automatically import data from different data sources. 
-- If you remove a column from the raw dataset, that's a huge problem
-- So, what you will to do, which is something I'd actually do in my real work, is that;
-- I am going to create some type of staging table of the raw data where my workings can be done
...
-- All you have to do now is insert the data into it 
...
-- It is because we are about to change a whole lot in the staging dataset
...
-- In SSMS, something similar was done but in that server, there was an extra column before the beginning of the data that gave a unique id number for each row
-- which made it so easy to remove duplicates
...
-- When you run this, you could see that there are three results, and just two are duplicates. The last one is not a duplicate.
...
-- Of course, I wouldn't want to delete both of them from the table because that would be bad
-- When we looked at Casper, we noticed that there is one we want to keep and one we want to take out
-- To do things like removing a duplicate, can be a little bit tricky in MySQL than it is in Microsoft Sql Server
...
-- So what we are going to do is take the duplicate_cte, probably put it into a staging2 database 
-- and then we can delete it because we can filter on the row_num and delete those which are equal to 2
...
-- Ensure your safe updates on MySQL is unchecked
...
-- You can easily view the error for a particular column by running a Distinct statement
...
-- Moving forward, when you look at the industries 'crypto', 'cryptocurrency', 'cryptocurrency', 
-- these are probably supposed to be under one category because they are supposedly the same thing
-- Why this change is necessary is because when we start doing the exploratory data analysis, when visualizing it, 
-- these would all be on their own rows and unique, which we don't want
...
FROM layoffs_staging2
ORDER BY 1;
-- We found one issue here where there was United States twice but someone put a period after one of them
-- We need to fix this
-- They are all meant to be United States and no United States.
-- So;
...
-- Now, just doing the TRIM won't fix it but here is what you can do
-- This is something called Trailing which means coming at the end
-- So what's trailing here? The period from country
